...
namespace Xml
{
	...
	template<typename T>
	inline bool DeserializeFromElement(Element parent, const std::wstring& name, T& val, bool innerXML)
	{
		try
		{
			...
			if(el != 0)
			{
				_bstr_t txt = innerXML ? el->xml : el->text;
				FromString((PCWSTR)txt, val);
				return true;
			}
		}
		catch(_com_error&)
	...
	inline bool Deserialize(Element parent, const std::wstring& name, std::wstring& val, bool fromElement = false, bool isXML = false)
	{
		return fromElement ?
			DeserializeFromElement(parent, name, val, isXML) :
			DeserializeFromAttribute(parent, name, val);
	}
	inline bool Deserialize(Element parent, const std::wstring& name, bool& val, bool fromElement = false, bool isXML = false)
	{
		return fromElement ?
			DeserializeFromElement(parent, name, val, isXML) :
			DeserializeFromAttribute(parent, name, val);
	}
	inline bool Deserialize(Element parent, const std::wstring& name, float& val, bool fromElement = false, bool isXML = false)
	{
		return fromElement ?
			DeserializeFromElement(parent, name, val, isXML) :
			DeserializeFromAttribute(parent, name, val);
...
#endif
	inline bool Deserialize(Element parent, const std::wstring& name, int& val, bool fromElement = false, bool isXML = false)
	{
		return fromElement ?
			DeserializeFromElement(parent, name, val, isXML) :
			DeserializeFromAttribute(parent, name, val);
	}
	inline bool Deserialize(Element parent, const std::wstring& name, unsigned short& val, bool fromElement = false, bool isXML = false)
	{
		return fromElement ?
			DeserializeFromElement(parent, name, val, isXML) :
			DeserializeFromAttribute(parent, name, val);
	...
	template<typename T>
	inline bool Deserialize(Element parent, const std::wstring& name, std::vector<T>& src, bool fromElement = false, bool isXML = false)
	{
		try
		{
			Element el = parent->selectSingleNode(name.c_str());
	...
	template<typename T>
	inline void Deserialize(T& o, const std::wstring& fileName, const std::wstring& rootElementXPath)
	{
		CoInitialize(NULL);
		{
			Document doc;
			if(SUCCEEDED(doc.CreateInstance(L"Msxml2.DOMDocument")))
			{
				...
				{
					Deserialize(doc->documentElement, rootElementXPath, o);
				}
			}
		}
		CoUninitialize();
